...
counts = rlps_data[rlps_data["MONTH"]==1]["GEMEENTE"].value_counts()

# Get unique values for dropdowns
gemeente_options = counts.index.tolist()
    ...
    dbc.Row([
        dbc.Col([html.Label("GEMEENTE:"),
                 dcc.Dropdown(
                     ...
                     value='EINDHOVEN',  # Default to first option
        ...
        dbc.Col([html.Label("MONTH:"),
                 dcc.Dropdown(
                     ...
                     value=7,  # Default to first option
                     clearable=False)], width=4),
...
def update_plot(selected_gemeente, selected_month, highlight_boxid):
    models = ['isomap', 'umap', 'sphere',   't-SNE']
    ...
    if highlight_boxid:
        rlps = rlps_data[(rlps_data['GEMEENTE'] == selected_gemeente) &
            ...
            columns=["GEMEENTE", "MONTH", "BOXID", "CLUSTER"], errors='ignore')
