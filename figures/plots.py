...
MODEL_NAMES = ['sphere', 'isomap', 'umap', 't-SNE', 'vae', 'profile']
...
outer = fig.add_gridspec(
    2, 3, wspace=0.2, hspace=0.0, left=0.1, bottom=0.05, right=0.9, top=0.95, width_ratios=widths, height_ratios=heights
)
...
for ii, (outer, model) in enumerate(zip(outers, MODEL_NAMES)):
    ...
    if ii != 5:
        ...
        sub_1 = gridspec.GridSpecFromSubplotSpec(3, 3, subplot_spec=outer, wspace=0.3, hspace=0.0)
        ...
        ax_a.set_xlabel("$z_1$", labelpad=-10, fontsize=8)
        ax_a.set_ylabel("$z_2$", labelpad=-10, fontsize=8)
        ax_a.set_zlabel("$z_3$", labelpad=-15, fontsize=8)
        ax_a.set_title(f"Model: {model}", fontsize=8)
        ...
        for combination, combination_label, ax_subplot in zip(combinations, combinations_labels, ax_subplots):
...
plt.suptitle("Municipality 1 - August")
